...
class Data:
    def __init__(self, config):
        ...
        connUrl = connUrlTemplate.format(
            user=config['user'], password=config['password'], dbname=db_name)
...
def updateBaseSummons(data, summons):
    for summon in summons:
        ...
        element = data.elements.find_one(
            {"name": summon['element'].capitalize().strip()})
        ...
        data.db['BaseSummon'].update_one({'name': s['name'], 'element': s['element'], 'rarity': s['rarity']}, {
                                         '$set': {'maxUncap': s['maxUncap'], 'baseUncap': s['baseUncap'], 'imgUrl': s['imgUrl']}}, upsert=True)
...
def updateBaseWeapons(data, weapons):
    ...
    for weapon in weapons:
        ...
        if weaponName in to_ignore:
            continue
        if "(element)" in weaponName.lower():
            continue

        element = data.elements.find_one(
            {"name": weapon['element'].capitalize().strip()})
        rarity = data.rarities.find_one({"name": weapon['rarity'].strip()})
        wType = data.weaponTypes.find_one(
            {"name": weapon['type'].capitalize().strip()})
        if not wType:
            continue
        if not element:
            continue
        ...
        data.db['BaseWeapon'].update_one({'name': w['name'], 'element': w['element'], 'rarity': w['rarity']}, {
                                         '$set': {'maxUncap': w['maxUncap'], 'baseUncap': w['baseUncap'], 'imgUrl': w['imgUrl'], 'weaponType': w['weaponType']}}, upsert=True)


def updateBaseCharacters(data, characters):
    for character in characters:
        element = data.elements.find_one(
            {"name": character['element'].capitalize().strip()})
        rarity = data.rarities.find_one(
            {"name": character['rarity'].upper().strip()})
        style = data.styles.find_one(
            {"name": character['type'].capitalize().strip()})
        race = []
        weapon = []

        rTokens = character['race'].split(',')
        for token in rTokens:
            r = data.races.find_one(
                {"name": token.strip().capitalize().strip()})
            race += [r['_id']]
        wTokens = character['weapon'].split(',')
        for token in wTokens:
            w = data.weaponTypes.find_one(
                {"name": token.strip().capitalize().strip()})
            weapon += [w['_id']]
        ...
        data.db['BaseCharacter'].update_one({'name': c['name'], 'element': c['element'], 'rarity': c['rarity']}, {
                                            '$set': {'maxUncap': c['maxUncap'], 'weaponType': c['weaponType'], 'imgUrl': c['imgUrl'], 'race': c['race'], 'style': c['style']}}, upsert=True)


def setBaseSummons(data):
    print("Fetching summon data...")
    summons = parseBaseData(
...
def setBaseWeapons(data):
    ...
    weapons = parseBaseData(
        'weapons', 'id,name,evo_base,evo_max,rarity,element,type')
    print("Preparing data and inserting in database...")
...
def setBaseCharacters(data):
    ...
    characters = parseBaseData(
        'characters', 'id,name,max_evo,rarity,element,type,race,weapon')
    print("Preparing data and inserting in database...")
